...
| `references/interaction-patterns.md` | Phase 2 (before first interactive phase) | Cognitive modes, refinement loop pattern, ask_user_question option design |
...
5. ask_user_question — "Do you have additional context to provide?" with options:
...
6. Based on response:
   ...
   - **URLs**: Collect URLs via ask_user_question (one question, user provides list). Store in `design_context.collected_urls`.
   - **Mini-research**: Collect research topics via ask_user_question. Store in `design_context.research_topics`.

7. Present detected characteristics with rationale for user confirmation:

ask_user_question — "I detected these design characteristics. Please confirm or correct:" with options:
...
1. inline skill `maister-codebase-analyzer` (to understand existing product context, tech stack, UI patterns)

**SELF-CHECK**: Did you invoke the inline skill `maister-codebase-analyzer`? Or did you start reading project files yourself? If the latter, STOP and invoke the inline skill loading.
...
3. Fetch external links collected in Phase 0 using `fetch_content` tool for each URL in `design_context.collected_urls`
4. If `design_context.research_topics` is non-empty: launch information-gatherer agents for each topic
   ...
   **INVOKE NOW** -- subagent tool call (parallel, one per topic):
   subagent tool - `maister-information-gatherer` subagent per research topic
...
6. ask_user_question — "Context synthesis complete. Key findings: [2-3 bullet summary]. Any corrections or additions before we explore the problem space?"
...
ask_user_question — "Problem space explored." Read `next_phase` from `orchestrator-state.yml`. If next phase is Phase 4, prepend "Skipping persona exploration (enhancement scope). " Ask "Continue to [next_phase value]?"
...
ask_user_question — one question at a time about user types and their needs
...
ask_user_question — "Personas defined. Continue to Idea Generation?"
...
subagent tool - `maister-solution-brainstormer` subagent
...
**ANTI-PATTERN -- DO NOT DO THIS:**
- Do NOT present all decision areas in a single summary table and ask one combined question. Each area MUST get its own detailed presentation and ask_user_question.
...
2. For each decision area sequentially:
   ...
   d. ask_user_question — alternatives as options (mark recommended with "(Recommended)") + "Need more info" + "Let me explain my thinking"
   e. Record choice, move to next area

> **SELF-CHECK before each ask_user_question**: Did you output the full alternatives with pros/cons for THIS area? If you only showed a recommendation line, STOP and output the full detail.
...
ask_user_question — "Design direction approved. Continue to Feature Specification?"
...
ask_user_question — "Specification complete." Read `next_phase` from `orchestrator-state.yml`. If next phase is Phase 8, prepend "No UI prototyping needed (backend-focused design). " Ask "Continue to [next_phase value]?"
...
**INVOKE NOW** -- subagent tool call:
subagent tool - `maister-ui-mockup-generator` subagent
...
ask_user_question — "Visual prototyping complete. Continue to Review & Handoff?"
